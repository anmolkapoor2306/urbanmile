...
export default function StatusButtons({ bookingId, currentStatus, onStatusChange }: { bookingId: string, currentStatus: string, onStatusChange?: (status: string) => void }) {
  ...
  const STATUSES = ['NEW', 'CONFIRMED', 'ASSIGNED', 'COMPLETE', 'CANCELLED']
  ...
  const STATUS_STYLES: Record<string, { active: string; inactive: string }> = {
    ...
    ASSIGNED: { active: 'bg-purple-500 text-white', inactive: 'border border-purple-500 text-purple-400' },
    COMPLETE: { active: 'bg-green-500 text-white', inactive: 'border border-green-500 text-green-400' },
    CANCELLED: { active: 'bg-red-500 text-white', inactive: 'border border-red-500 text-red-400' },
  }
  ...
  const handleClick = async (status: string) => {
    ...
    try {
      const apiStatus = status === 'COMPLETE' ? 'COMPLETED' : status
    ...
    <div className="flex flex-nowrap gap-1.5">
      {STATUSES.map((status) => {
        ...
        return (
          ...
          >
            {status === 'NEW' ? 'New/Unassigned' : status.charAt(0) + status.slice(1).toLowerCase()}
          </button>
        )
      })}
    </div>
